...
    // Initial state setup helper to update both state and immediate refs
...
            viewportRef.current = maps[0].viewport;
        }
        setMapId(mid);
...
                    if (currentMapId) {
                        DatabaseService.maps.update({ id: currentMapId, userId, viewport: updatedViewport, updatedAt: Date.now() });
                    }
                }
...
            if (mapId) DatabaseService.maps.update({ id: mapId!, userId, viewport: viewportRef.current, updatedAt: Date.now() });
        } 
    };
...
                    return n;
                });
                nodesRef.current = next; // Sync ref synchronously to avoid race condition on pointerup
                return next;
            });
...
            if (mapId) DatabaseService.maps.update({ id: mapId!, userId, viewport: viewportRef.current, updatedAt: Date.now() });
        } else if (interactionMode === 'DRAGGING') {
            // FIX: PERSIST POSITIONS TO DATABASE ON DRAG END
            const currentNodes = nodesRef.current;
            // Explicitly type 'n' to ensure n.id and other properties are correctly accessed as strings/objects
            // Each fix: Explicitly type n as MapNodeEntity to avoid 'unknown' issues
...
                    const firstId = idArray.length > 0 ? idArray[0] : null;
...
            if (mapId) DatabaseService.maps.update({ id: mapId!, userId, viewport: nextViewport, updatedAt: Date.now() });
        }
    };
...
            
            <div className="absolute bottom-6 right-6 z-20 flex gap-2 bg-white/90 dark:bg-slate-900/90 backdrop-blur p-1.5 md:p-2 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700"><button onClick={() => setViewport(v => { const next = {...v, zoom: Math.max(ZOOM_MIN, v.zoom - 0.1)}; viewportRef.current = next; return next; })} className="p-1.5 md:p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg"><Minus size={14} className="md:w-4 md:h-4" /></button><span className="py-1.5 md:py-2 min-w-[2.5rem] md:min-w-[3rem] text-center text-[10px] md:text-xs font-bold text-slate-500">{Math.round(viewport.zoom * 100)}%</span><button onClick={() => setViewport(v => { const next = {...v, zoom: Math.min(ZOOM_MAX, v.zoom + 0.1)}; viewportRef.current = next; return next; })} className="p-1.5 md:p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg"><Plus size={14} className="md:w-4 md:h-4" /></button></div>
